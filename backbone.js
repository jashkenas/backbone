...
(function () {
    ...
    var Base = Backbone.Base = function () {
    };
    ...
    var Model = Backbone.Model = State.extend({
        constructor: function (attributes, options) {
    ...
    // Backbone BaseViews are almost more convention than they are actual code. A BaseView
    // is simply a JavaScript object that represents a logical chunk of UI in the
    // DOM. This might be a single item, an entire list, a sidebar or panel, or
    // even the surrounding frame which wraps your whole app. Defining a chunk of
    // UI as a **View** allows you to define your DOM events declaratively, without
    // having to worry about render order.

    // Backbone BaseView instances are generic MVC/MVP pattern views. They are 'controlled'
    // by the object containing the view instance and they have no reference to any
    // Backbone.Model or Backbone.Collection instance.

    // Options with special meaning *(e.g. model, collection, id, className)* are
    // attached directly to the view.  See `baseViewOptions` for an exhaustive
    // list.

    // Creating a Backbone.BaseView creates its initial element outside of the DOM,
    // if an existing element is not provided...

    // Set up all inheritable **Backbone.BaseView** properties and methods.
    var BaseView = Backbone.BaseView = Base.extend({
        constructor: function (options) {
            this.cid = _.uniqueId('view');
            options || (options = {});
            _.extend(this, _.pick(options, BaseView.viewOptions));
            this._ensureElement();
            this.initialize.apply(this, arguments);
            this.delegateEvents();
        },

        // The default `tagName` of a BaseView's element is `"div"`.
        tagName: 'div',
        ...
        // Ensure that the BaseView has a DOM element to render into.
    ...
    BaseView.viewOptions = ['el', 'id', 'attributes', 'className', 'tagName', 'events'];
    ...
    var Collection = Backbone.Collection = Base.extend({
    ...
    var Router = Backbone.Router = Base.extend({
        constructor: function (options) {
            options || (options = {});
    ...
    Backbone.sync = function (method, model, options) {
        ...
        if (options.emulateHTTP && (type === 'PUT' || type === 'DELETE' || type === 'PATCH')) {
            ...
            options.beforeSend = function (xhr) {
                ...
                if (beforeSend) result = beforeSend.apply(this, arguments);
                return result;
            };
        }
